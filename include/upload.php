...
function _asUpFiles($dir, $file_var, $max_size='', $type='', $name=false) 
{
	if (!file_exists($dir))
	{
	showmsg("上传文件失败：上传目录 ".$dir." 不存在!",0);
	}
	elseif (!is_writable($dir)) 
	{
	showmsg("上传文件失败：上传目录 ".$dir." 无法写入!",0);
	exit(); 
	}
	...
	if (!empty($upfilename)) 
	{
		...
		{ 
		showmsg('上传文件失败：你选择的文件无法上传',0);
		exit(); 
		} 
		elseif ($max_size>0 && $upfile['size']/1024>$max_size) 
		{ 
		showmsg("上传文件失败：文件大小不能超过  ".$max_size."KB",0);
		exit(); 
		}
		...
		if (!empty($type))
		{
			...
			{
			showmsg("上传文件失败：只允许上传 ".$type." 的文件！",0);
			exit(); 
			}
