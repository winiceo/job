...
if (isset($aset['page']))
{
	...
	$result = $db->query("SELECT id,uname,detailed,refreshtime,category,is_hot,sex,age,experience_cn,deadline,district_cn,sdistrict_cn FROM ".table('simple_resume')." ".$wheresql.$orderbysql.$limit);
	$list   = array();
	//echo "SELECT * FROM ".table('jobs')." ".$wheresql.$orderbysql.$limit;
		...
		$row['simple_url']=url_rewrite($aset['simpleresumeshow'],array('id'=>$row['id']));
