...
$sql = "select id,title,holddate_start,holddate_end,predetermined_status,predetermined_start,predetermined_end,trade_cn,bus,introduction,predetermined_web,contact,phone,address,service_setmeal,price,map_x,map_y,map_zoom from ".table('jobfair')." WHERE  id=".intval($aset['id'])." AND  display=1 LIMIT 1";
$val=$db->getone($sql);
if (empty($val))
{
