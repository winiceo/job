...
{		
		...
		if ($_COOKIE['QS']['utype']=='4' && $_SESSION['utype']=='4')
		{
			$user_url=$_CFG['site_dir']."user/train/train_index.php";
			$course=$db->get_total("SELECT COUNT(*) AS num FROM ".table('course'). " WHERE uid={$_SESSION['uid']}");
			$apply_course=$db->get_total("SELECT COUNT(*) AS num FROM ".table('personal_course_apply')."  WHERE train_uid={$_SESSION['uid']}");
			$teachers=$db->get_total("SELECT COUNT(*) AS num FROM ".table('train_teachers')." WHERE uid={$_SESSION['uid']} ");
			$count_htm='<div class="core-function f-left">
		...
		if ($_COOKIE['QS']['utype']=='3' && $_SESSION['utype']=='3')
		{
			$user_url=$_CFG['site_dir']."user/hunter/hunter_index.php";
			$interview=$db->get_total("SELECT COUNT(*) AS num FROM  ".table('hunter_interview')."  as i  WHERE i.hunter_uid={$_SESSION['uid']}");
			$count_apply=$db->get_total("SELECT COUNT(*) AS num FROM ".table('personal_hunter_jobs_apply')." AS a   WHERE a.huntet_uid='{$_SESSION['uid']}'");
			$dow_num=$db->get_total("SELECT COUNT(*) AS num FROM ".table('hunter_down_resume')." as d WHERE  d.hunter_uid='{$_SESSION['uid']}'");
...
{		
	require_once(QISHI_ROOT_PATH.'include/fun_user.php');
	$contents='';
	if ($_COOKIE['QS']['username'] && $_COOKIE['QS']['password'] && $_SESSION['uid'] && $_SESSION['utype']=='2')
	{
	...
	if ($_COOKIE['QS']['utype']=='2' && $_SESSION['utype']=='2')
	{
		...
		if(intval($user_info['email_audit']) == 1)
		{
			$email_html = '<div class="auth-item mail">'.$user_info['email'].'</div>';
		}
		else
		{
			$email_html = '<div class="auth-item mail-noauth">未认证</div>';
		}
		if(intval($user_info['mobile_audit']) == 1)
		{
			$mobile_html = '<div class="auth-item phone">'.$user_info['mobile'].'</div>';
		}
		else
		{
			$mobile_html = '<div class="auth-item phone-noauth">未认证</div>';
		}
		$contents=str_replace('{#$email#}',$email_html,$contents);
		$contents=str_replace('{#$mobile#}',$mobile_html,$contents);
		$my_eval = url_rewrite('QS_my_evaluation');
		$count_htm='<a href="javascript:;" class="menu-item f-left rank">测评排名</a><a href="javascript:;" class="menu-item f-left book">测评宝典</a><a href="'.$my_eval.'" class="menu-item f-left my">我的测评</a>';
		$contents=str_replace('{#$count#}',$count_htm,$contents);
